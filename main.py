...
def get_notes(u_acc, ex_id):
    ...
    try:
        ...
        for doc in docs:
            # Lấy chính xác phần g_id ở cuối ID (ví dụ: lấy '1' từ 'user_exID_1')
            gid_key = doc.id[len(prefix):]
            notes_dict[gid_key] = doc.to_dict().get('content', "")
...
# --- 6. GIAO DIỆN TRANG CHỦ & ĐIỀU HƯỚNG ---
def login_page():
    st.markdown('<h1 style="text-align: center;">🔑 Đăng nhập Hệ thống</h1>', unsafe_allow_html=True)
    with st.container(border=True):
        acc = st.text_input("📧 Tài khoản của bạn:")
        pwd = st.text_input("🔒 Mật khẩu:", type="password")
        if st.button("Xác nhận", use_container_width=True):
            ref = db.collection('users').document(acc).get()
            if ref.exists and str(ref.to_dict().get('password')) == pwd:
                st.session_state.user = {**ref.to_dict(), 'account': acc}
                st.rerun()
...
def student_page():
    ...
    if st.session_state.view_mode == 'list':
        ...
        for item in ex_list:
            ...
            with st.container(border=True):
                ...
                with c1:
                    ...
                    date_str = item['created_at'].strftime("%d/%m/%Y") if hasattr(item['created_at'], 'strftime') else "N/A"
        ...
        for g_id, group_df in df.groupby('group'):
            ...
            if str(first.get('aud_tmp')).strip() != "": display_drive_audio(first.get('aud_tmp'))
            
            # --- PHẦN GHI CHÚ CHÍNH XÁC ---
            gid_str = str(g_id)
            # Lấy giá trị từ st.session_state.user_notes (đã nạp trong callback)
            saved_val = st.session_state.user_notes.get(gid_str, "")
            
            # Ô GHI CHÚ: Dùng value để hiện dữ liệu cũ, key phải duy nhất kèm ID bài tập
            note_input = st.text_area("📝 Ghi chú / Chiến thuật (Tự động lưu khi nhấn ra ngoài):", value=saved_val, key=f"note_quiz_{st.session_state.current_ex_id}_{gid_str}", height=150, max_chars=500)
            
            # Chỉ cập nhật lên Firebase nếu người dùng thực sự thay đổi nội dung
            ...
            ctx, l, r_col = clean_nan(first.get('context')), *st.columns([1, 1])
            with l:
            ...
            with r_col:
                with st.container(height=900):
                    for i, r in group_df.iterrows():
                        ...
                        sel = st.radio(f"q{i}", opts, key=f"r_{st.session_state.current_ex_id}_{i}", index=opts.index(cur_v) if cur_v in opts else None, label_visibility="collapsed")
                        if sel != cur_v:
        ...
        for g_id, group_df in df.groupby('group'):
            ...
            # HIỂN THỊ GHI CHÚ TRONG REVIEW (ÉP DỮ LIỆU TỪ DATABASE)
            gid_str = str(g_id)
            saved_note = st.session_state.user_notes.get(gid_str, "")
            note_rev = st.text_area("📝 Ghi chú cũ của bạn:", value=saved_note, key=f"note_rev_{st.session_state.current_ex_id}_{gid_str}", height=150, max_chars=500)
            ...
            ctx, l_rev, r_rev = clean_nan(first.get('context')), *st.columns([1, 1])
            with l_rev:
                with st.container(height=900):
                    for p in ctx.split(";;"):
                        if p.strip().startswith("http"): display_drive_image(p.strip())
            ...
            with r_rev:
                with st.container(height=900):
                    for i, r in group_df.iterrows():
                        ...
                        if 'transcript' in df.columns and clean_nan(r.get('transcript')) != " ":
